...
int init(int numberOf) {
  ...
  } else if ((input[0] == triangleExpect[0]) ||
  ...
  if (answer_point == 2) {
    ...
    for (int k = 0; k < 4; k++) {
      printf("Triangle's x%d = %.2lf, y%d = %.2lf\n", k + 1,
             fig_obj[figure_num].p[k].x, k + 1, fig_obj[figure_num].p[k].y);
    }
    fig_obj[figure_num] = format_figure(&fig_obj[figure_num]);
...
Figure format_figure(Figure *fig) {
  if (fig->type == 1) {
    double first_side = sqrt(SQR((fig->p[1].x - fig->p[0].x)) +
                             SQR((fig->p[1].y - fig->p[0].y)));
    double second_side = sqrt(SQR((fig->p[2].x - fig->p[1].x)) +
                              SQR((fig->p[2].y - fig->p[1].y)));
    double third_side = sqrt(SQR((fig->p[3].x - fig->p[2].x)) +
...
int figure_intersection(Figure *fig, Figure *fig2) {
  if ((fig->type == 0) && (fig2->type == 0)) {
    double d =
        (fig->center.x - fig2->center.x) * (fig->center.x - fig2->center.x) +
  ...
  if ((fig->type == 1) && (fig2->type == 1)) {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        ff = cross_segment(fig->p[i].x, fig->p[i].y, fig->p[i + 1].x,
                           fig->p[i + 1].y, fig2->p[j].x, fig2->p[j].y,
                           fig2->p[j + 1].x, fig2->p[j + 1].y);
      }
    }
    if (ff) {
      return 1;
